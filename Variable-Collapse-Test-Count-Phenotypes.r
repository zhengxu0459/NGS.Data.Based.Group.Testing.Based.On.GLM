# 
# R Script file to implement our variable collapse test for count phenotype
# Version 1.2 (continuously updated)
# Author: Zheng Xu

variable.collapse.test.count.phenotype = function(pheno,snp.glf,
  ...
  sub.n.covariate = dim(extended.cov)[2]
	...
	# score statistic and p value for variable collapse test for count phenotype
	Collapsed.Score = 
		t(Collapsed.Reduced.Score.vector)%*%solve(Collapsed.Reduced.Hessian.matrix)%*%Collapsed.Reduced.Score.vector 
	collapsed.p.value=
		1-pchisq(Collapsed.Score,df=1)
		
	return(list( df=sub.n.marker, score.statistic=Collapsed.Score, 
		         p.value=collapsed.p.value))
} 